...
public interface MedicoRepository extends JpaRepository<Medico, Long> {
    ...
    @Query("select m from Medico m where m.activo= true and m.especialidad=:especialidad and m.id not in(select c.medico.id from Consulta c where c.fecha=:fecha) order by rand() limit 1")
